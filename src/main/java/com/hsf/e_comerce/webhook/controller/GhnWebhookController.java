package com.hsf.e_comerce.webhook.controller;

import com.hsf.e_comerce.order.service.OrderService;
import com.hsf.e_comerce.webhook.dto.GhnWebhookPayload;
...
public class GhnWebhookController {
    ...
    private static final int STATUS_DELIVERED_ID = 5;
    ...
    public ResponseEntity<Void> handleGhnWebhook(@RequestBody GhnWebhookPayload payload) {
        ...
        boolean isDelivered = STATUS_DELIVERED_ID == (payload.getDeliveryStatusId() != null ? payload.getDeliveryStatusId() : -1)
                || isDeliveredStatus(payload.getDeliveryStatus());

        if (isDelivered && (orderCode != null || clientOrderCode != null)) {
            try {
                boolean updated = orderService.markDeliveredByGhnRef(orderCode, clientOrderCode);
                if (updated) {
